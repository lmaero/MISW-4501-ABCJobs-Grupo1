...
export async function getAllResults (req: Request, res: Response) {
  try {
    ...
    if (resultsForAllCandidates) {
      return res.status(200).json({ resultsForAllCandidates: resultsForAllCandidates.tests })
    } else {
    ...
    if (candidateId) {
      ...
      if(dbResult.msg === "201") {
        ...
        for(let prueba in candidateDbResults) {
          ...
          const updateResponse = await dao.updateCandidateTestScore(candidateId,  resultsWithScore.id.toString(), Number(score))
