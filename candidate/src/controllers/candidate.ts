...
const getTests = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const { userInfo: { personId } } = JSON.parse(user.toString());
        
        console.log(`[CandidateManagment] Getting tests for candidate ${personId}, from evaluator service`);
        const tests: Test[] = await getTestsByUser(Number(personId));
        
        return res.status(200).json({ message: "test evaluated successfully", tests });
    } catch (error) {
        console.dir(error);
